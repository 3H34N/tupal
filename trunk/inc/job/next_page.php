...
$rs=$db->get_one("SELECT aid,fid FROM {$pre}article$erp WHERE fid=$fid AND aid<$aid ORDER BY aid DESC LIMIT 1");
...
if($rs[aid]){
	header("location:$webdb[www_url]$webdb[path]/bencandy.php?fid=$fid&aid=$rs[aid]");exit;
}else{
	header("location:$webdb[www_url]$webdb[path]/list.php?fid=$fid");exit;
	//showerr("已到尽头了");
}
?>