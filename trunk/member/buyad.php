...
elseif($action=="buy")
{
	...
	*/
	...
	make_ad_cache();
	refreshto("?job=list","购买成功,你共支付了{$u_moneycard}金币","3");
}
...
elseif($action=="edit")
{
	...
	if($rsdb[autoyz])
	{
		...
		else
		{
			...
			$db->query("UPDATE {$pre}memberdata SET moneycard=moneycard-'$money' WHERE uid='$lfjuid'");
		}			
	}
	else
	{
	...
	$u_hits=0;
	$db->query("UPDATE `{$pre}ad_user` SET `u_day`='$atc_day',`u_begintime`='$u_begintime',`u_endtime`='$u_endtime',`u_yz`='$u_yz',`u_code`='$u_code',`u_moneycard`='$totalmoneycard' WHERE u_id='$u_id' AND u_uid='$lfjuid'");
