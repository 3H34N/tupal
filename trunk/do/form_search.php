...
if( $_GET[keyword] || $_POST[keyword] )
{
	...
	elseif($type&&$mid&&is_table("{$pre}form_content_$mid")&&table_field("{$pre}form_content_$mid",$type))
	{
		$SQL.=" AND BINARY C.$type LIKE '%$keyword%' ";
		$search_module=1;
	}

	if($mid&&is_table("{$pre}form_content_$mid"))
	{
		$search_url='';
		foreach( $postdb AS $key=>$value)
		{
			if( $value!='' && table_field("{$pre}form_content_$mid",$key) )
			{
				$SQL.=" AND C.`$key`='$value' ";
	...
	if($search_module==1)
	{
		$_moduleSql=" LEFT JOIN {$pre}form_content_$mid C ON A.id=C.id ";
		$_moduleSqlSearch=" C.*, ";
	}
	
	$showpage=getpage("{$pre}form_content A $_moduleSql","WHERE $SQL","?type=$type&$search_url&keyword=".urlencode($_POST[keyword]).urlencode($_GET[keyword])."",$rows);

	unset($listdb);

	$query = $db->query("SELECT {$_moduleSqlSearch}A.* FROM {$pre}form_content A $_moduleSql WHERE $SQL  ORDER BY A.id DESC LIMIT $min,$rows");
...
require(PHP168_PATH."php168/form_tpl/search_$mid.htm");
