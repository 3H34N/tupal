...
if($job=="js")
{
	...
	$query = $db->query("SELECT C.about,C.type,C.votetype,C.tplcode,V.* FROM {$pre}vote_config C LEFT JOIN {$pre}vote V ON C.cid=V.cid $SQL ORDER BY V.list DESC");
	...
	if(!$tplcode){
		require(PHP168_PATH."template/default/vote_js.htm");
	}else{
		if(!$votetype){
print<<<EOT
	<div id="vote_js">
	  <form name="formv" method="post" action="$webdb[www_url]/do/vote.php?action=vote&cid=$cid" target="_blank">
		...
		<div class="votepost"> 
		  ...
		  &nbsp;&nbsp;
		  <input type="button" name="Submit2" title="查看投票" value="" onclick="window.open('$webdb[www_url]/do/vote.php?job=show&cid=$cid')" style="background:url('$webdb[www_url]/images/default/vote_view.gif');width:44px;height:21px;border:0px;">
		</div>
	  </form>
	</div>
EOT;
...
print<<<EOT
		<table width="100%" border="0" cellspacing="0" cellpadding="0" id="vote_js"><form name="formv" method="post" action="$webdb[www_url]/do/vote.php?action=vote&cid=$cid" target="_blank">
		  <tr> 
      ...
      <td class="votepost" align="center"> 
        <input type="submit" name="Submit" title="我要投票"  value="" style="background:url('$webdb[www_url]/images/default/vote_sub.gif');width:44px;height:21px;border:0px;">&nbsp;&nbsp;<input type="button" name="Submit2" title="查看投票" value="" onclick="window.open('$webdb[www_url]/do/vote.php?job=show&cid=$cid')" style="background:url('$webdb[www_url]/images/default/vote_view.gif');width:44px;height:21px;border:0px;">
      </td>
		  </tr></form>
		</table>
EOT;
...
elseif($action=="vote")
{
	...
	@extract($db->get_one("SELECT cid FROM {$pre}vote WHERE id='$voteId[0]'"));
	...
	if($rsdb[limitip])
	{
		...
		$db->query("UPDATE {$pre}vote_config SET ip='$rsdb[ip]' WHERE cid='$cid'");
	}
	if($rsdb[limittime])
	{
	...
	{
		$db->query("UPDATE {$pre}vote SET votenum=votenum+1 WHERE id='$value' ");
	}
...
elseif($job=="show")
{
	...
	$query = $db->query("SELECT C.about,C.type,C.ifcomment,V.* FROM {$pre}vote_config C LEFT JOIN {$pre}vote V ON C.cid=V.cid WHERE C.cid='$cid' ORDER BY V.list DESC");
	$total=0;
	while($rs = $db->fetch_array($query)){
		$total=$total+$rs[votenum];
		if($rs[type]==1){
			$rs[type]="<input type='radio' name='voteId' value='$rs[id]' style='border:0px;'>";
		}else{
			$rs[type]="<input type='checkbox' name='voteId[]' value='$rs[id]' style='border:0px;'>";
		}
		$about=$rs[about];
...
elseif($job=="shownum")
{
	@extract($db->get_one("SELECT votenum FROM {$pre}vote WHERE id='$id'"));
