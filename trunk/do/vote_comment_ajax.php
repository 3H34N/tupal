...
if($action=="post"){
	...
	$rss=$db->get_one(" SELECT * FROM {$pre}vote_config WHERE cid='$cid' ");
	if(!$rss)
	{
		die("原数据不存在");
	}
	...
	if($allow)
	{
		...
		$cnvert = new Chinese("GB2312","UTF8",$content,PHP168_PATH."./inc/gbkcode/");
		$content = $cnvert->ConvertIT();
		if(!$lfjid){
			$cnvert = new Chinese("GB2312","UTF8",$username,PHP168_PATH."./inc/gbkcode/");
...
$showpage=preg_replace("/\?cid=([\d]+)&page=([\d]+)/is","javascript:getcomment('$webdb[www_url]/do/vote_comment_ajax.php?cid=\\1&page=\\2')",$showpage);

require_once(html('vote_comment_ajax'));
