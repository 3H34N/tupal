...
elseif($action=="copysina"&&$Apower[gather_copysina])
{
	...
	if($titledb[$page]){
					...
					if($copypic==1){
						...
						if( @copy($atc_picnews,PHP168_PATH."$webdb[updir]/article/$fid/$name") ){
							...
							if(!$picurl){
								...
								if($webdb[if_gdimg]){
									gdpic(PHP168_PATH."$webdb[updir]/article/$fid/$name",$Newpicpath,300,225,$webdb[autoCutSmallPic]?array('fix'=>1):'');

									gdpic(PHP168_PATH."$webdb[updir]/article/$fid/$name","{$Newpicpath}.jpg",225,300,$webdb[autoCutSmallPic]?array('fix'=>1):'');

									gdpic(PHP168_PATH."$webdb[updir]/article/$fid/$name","{$Newpicpath}.jpg.jpg",300,300,$webdb[autoCutSmallPic]?array('fix'=>1):'');
								}
		...
		$db->query("INSERT INTO `{$pre}reply$erp` (  `aid` , `fid` ,`uid` ,  `content` ,topic) VALUES ( '$rs[aid]', '$fid','$userdb[uid]', '$lfj_concent',1)");
