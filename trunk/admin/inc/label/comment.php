...
if($action=='mod')
{
	...
	$SQL=" SELECT A.*,A.content AS title,B.title AS article FROM {$pre}comment A LEFT JOIN {$pre}article B ON A.aid=B.aid $SQL ORDER BY A.cid DESC LIMIT $rows ";
	...
	require("head.php");
	require("template/label/comment.htm");
	require("foot.php");
