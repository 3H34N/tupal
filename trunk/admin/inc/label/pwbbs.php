...
if($action=='mod'){
	...
	if(strstr($postdb[tplpart_2code],'$picurl')&&!strstr($postdb[tplpart_1code],'$picurl')){
		if(strstr($postdb[tplpart_2code],'$content')){
			$SQL2=" SELECT T.*,T.tid AS id,T.author AS username,T.authorid AS uid,T.subject AS title,T.postdate AS posttime,C.content,A.attachurl$_SQL1 FROM {$TB_pre}attachs A LEFT JOIN {$TB_pre}threads T ON A.tid=T.tid LEFT JOIN {$TB_pre}tmsgs C ON T.tid=C.tid$_SQL2 $SQL AND A.type='img' GROUP BY A.tid ORDER BY T.$order $asc,T.tid DESC LIMIT 1 ";
		}else{
			$SQL2=" SELECT T.*,T.tid AS id,T.author AS username,T.authorid AS uid,T.subject AS title,T.postdate AS posttime,A.attachurl$_SQL1 FROM {$TB_pre}attachs A LEFT JOIN {$TB_pre}threads T ON A.tid=T.tid$_SQL2 $SQL AND A.type='img' GROUP BY A.tid ORDER BY T.$order $asc,T.tid DESC LIMIT 1 ";
		}
		$postdb[sql2]=$SQL2;
	}
	...
	if(strstr($postdb[tplpart_1code],'$picurl')&&(strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content'))){
		$SQL=" SELECT T.*,T.tid AS id,T.author AS username,T.authorid AS uid,T.subject AS title,T.postdate AS posttime,C.content,A.attachurl$_SQL1 FROM {$TB_pre}attachs A LEFT JOIN {$TB_pre}threads T ON A.tid=T.tid LEFT JOIN {$TB_pre}tmsgs C ON T.tid=C.tid$_SQL2 $SQL AND A.type='img' GROUP BY tid ORDER BY T.$order $asc,T.tid DESC LIMIT $rows ";
	}elseif(strstr($postdb[tplpart_1code],'$picurl')||$stype=='r'){
		$SQL=" SELECT T.*,T.tid AS id,T.author AS username,T.authorid AS uid,T.subject AS title,T.postdate AS posttime,A.attachurl$_SQL1 FROM {$TB_pre}attachs A LEFT JOIN {$TB_pre}threads T ON A.tid=T.tid$_SQL2 $SQL AND A.type='img' GROUP BY tid ORDER BY T.$order $asc,T.tid DESC LIMIT $rows ";
	}elseif( strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content') ){
		$SQL=" SELECT T.*,T.tid AS id,T.author AS username,T.authorid AS uid,T.subject AS title,T.postdate AS posttime,C.content$_SQL1 FROM {$TB_pre}threads T LEFT JOIN {$TB_pre}tmsgs C ON T.tid=C.tid$_SQL2 $SQL ORDER BY T.$order $asc,T.tid DESC LIMIT $rows ";
	}else{
		$SQL=" SELECT T.*,T.tid AS id,T.author AS username,T.authorid AS uid,T.subject AS title,T.postdate AS posttime$_SQL1 FROM {$TB_pre}threads T$_SQL2 $SQL ORDER BY T.$order $asc,T.tid DESC LIMIT $rows ";
	}
