...
elseif($action=='mod'){
	...
	*/
	...
	if($s_type=='group'){
		...
		$SQL=" SELECT *,webname AS title,intro AS content,logo AS picurl FROM {$_pre}group $SQL ORDER BY $_order $asc LIMIT $rows ";
	}elseif($s_type=='bbs'){
		$_order=$order;
		if($order=='id'){
			$_order='tid';
		}
		$SQL=" SELECT *,tid AS id FROM {$_pre}forum_topic $SQL ORDER BY $_order $asc LIMIT $rows ";
	}else{
		$SQL=" SELECT * FROM {$_pre}photo_pic $SQL ORDER BY $order $asc LIMIT $rows ";
	}
