...
if($action=='mod'){
	...
	$SQL=" SELECT A.*,R.*,config AS M_config FROM {$pre}form_content A LEFT JOIN {$pre}form_content_{$amodule} R ON A.id=R.id LEFT JOIN {$pre}form_module F ON A.mid=F.id $SQL ORDER BY A.id $asc  LIMIT $rows ";
