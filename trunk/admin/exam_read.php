...
if($job=='list')
{
	...
	$query = $db->query("SELECT SQL_CALC_FOUND_ROWS F.* FROM `{$pre}exam_student` ST LEFT JOIN {$pre}exam_form F ON ST.form_id=F.id $SQL GROUP BY ST.form_id LIMIT $min,$rows");
	...
	$showpage=getpage("","","?lfj=$lfj&job=list","$rows",$totalNum);
...
elseif($job=="liststudent")
{
	...
	$showpage=getpage("`{$pre}exam_student`","WHERE form_id='$id'","?lfj=$lfj&job=$job&id=$id","$rows");
