...
if($job=="config"&&$Apower[center_config])
{
	...
	@include(PHP168_N."php168/mgroup/{$P_M[value][m1_groupid]}.php");
...
elseif($action=="config"&&($Apower[center_config]||$Apower[center_cache]||$Apower[user_reg]))
{
	...
	//当取消静态的时候,把首页重命名,不影响访问
	if(isset($webdbs[MakeIndexHtmlTime])&&!$webdbs[MakeIndexHtmlTime]&&$webdb[MakeIndexHtmlTime])
	{
		$rs=$db->get_one("SELECT htmlname FROM {$pre}channel WHERE id=1");
		@unlink(PHP168_PATH."index.htm.bak");
		rename(PHP168_PATH."$rs[htmlname]",PHP168_PATH."index.htm.bak");
	}
	//当更换风格后,如果是首页静态的,要删除他,不然的话看不到效果
	if( $webdb[MakeIndexHtmlTime] && isset($webdb[style]) )
	{
		$rs=$db->get_one("SELECT * FROM {$pre}channel WHERE id=1");
		if($webdb[style]!=$webdbs[style] || $index_style!=$rs[style])
		{
			@unlink(PHP168_PATH."index.htm.bak");
			rename(PHP168_PATH."$rs[htmlname]",PHP168_PATH."index.htm.bak");
		}
	...
	{
		$db->query("UPDATE {$pre}channel SET style='$index_style' WHERE id=1");
	}
...
elseif($job=="map")
{
	...
	@extract($db->get_one("SELECT COUNT(id) AS Guestbook FROM {$pre}guestbook"));
	...
	if( mysql_get_server_info() > '4.1' && $webdb[passport_type] ){
		...
		if($c1&&$c2&&($c1!=$c2)){
			echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
			echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,整合论坛失败,因为你论坛使用的数据库编码是:$c1,而整站使用的数据库编码是:$c2,这是不允许的，必须使整站的数据库编码与论坛的一样，现在你必须备份整站数据库,然后修改数据库配置文件php168/mysql_config.php数据库编码为$c1，然后再到地址栏还原数据库选择编码为:$c1,只能在地址栏还原.不能在后台还原.因为后台还原不能选择数据库编码,以上操作步骤绝对不能颠倒,即第一步整站后台备份数据,第二步修改数据库配置文件,第三步地址栏目还原数据库</A></CENTER>";
			exit;
		}
	}
	...
	if( mysql_get_server_info() > '4.1' ){
		...
		if($c2&&$c2!=$dbcharset){
			echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
			echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,你的数据库编码是:$c2,你必须修改文件/php168/mysql_config.php，在他倒数第二行加上这句话(注意结尾分号不能少):\$dbcharset = '$c2';</A></CENTER>";
			exit;
		}
	}
	...
	if( mysql_get_server_info() < '4.1' && $dbcharset){
		echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
		echo "<CENTER><A style='color:red;font-size:25px;'>严重警告,你的数据库是低版本，你必须修改文件/php168/mysql_config.php，在他倒数第二行加上这句话(注意结尾分号不能少):\$dbcharset = '';</A></CENTER>";
		exit;
	}
	
	if( eregi("^pwbbs",$webdb[passport_type]) && $webdb[passport_TogetherLogin] )
	{
		@include(PHP168_PATH."$webdb[passport_path]/data/bbscache/config.php");
	...
	{
		@include(PHP168_PATH."$webdb[passport_path]/config.inc.php");
