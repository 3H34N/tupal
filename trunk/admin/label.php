...
if($job=="getTplcode"){
	...
	$code=read_file(PHP168_PATH.$filepath);

	$code2=read_file(str_replace(".htm",".txt",PHP168_PATH.$filepath));
...
if($inc){
	...
	if( ereg("^(log|down|photo|mv|shop|music|flash)$",$inc) ){
		include(dirname(__FILE__)."/"."inc/label/c.php");
	}
	elseif(ereg("^Info_",$inc)){//Info_fenlei_
		$detail=explode("Info_",$inc);
		$_pre="{$pre}$detail[1]";
		$ModuleDB[$detail[1]]['admindir'] || $ModuleDB[$detail[1]]['admindir']='admin';
		$label_file=PHP168_PATH.$ModuleDB[$detail[1]]['dirname'].'/'.$ModuleDB[$detail[1]][admindir].'/label.inc.php';
		if($inc=='Info_group_'){
			include(dirname(__FILE__)."/"."inc/label/group.php");
		}
		elseif( is_file($label_file) ){
			include($label_file);
		}
		else{
			include(dirname(__FILE__)."/"."inc/label/info.php");
		}
	}
	else{
		include(dirname(__FILE__)."/"."inc/label/$inc.php");
	}
}elseif($job=="list"){
...
elseif($job=="make")
{
	...
	elseif($step=='getpic')
	{
		$dir=opendir(PHP168_PATH."images/diy.style/title_icon/");
...
function get_label(){
	...
	$rsdb=$db->get_one("SELECT * FROM {$pre}label WHERE ch='$ch' AND tag='$tag' AND pagetype='$ch_pagetype' AND module='$ch_module' AND fid='$ch_fid' AND chtype='$chtype' ");
...
function label_hf($tag,$_value){
	...
	write_file(PHP168_PATH."php168/label_hf.php",$show);
}
...
function make_tag_cache(){
	...
	$show="<?php\r\n";
	$query = $db->query("SELECT * FROM {$pre}label WHERE module='$ch_module' AND style='$mystyle' ");
	while($rs = $db->fetch_array($query)){
		$rs[code]=addslashes( $rs[code]);
		$rs[divcode]=addslashes( $rs[divcode]);
		$show.="\r\n\$TagDB['{$rs[tag]}']['{$rs[pagetype]}']=array(
	...
	if(!file_exists(PHP168_PATH."$rsdb[dirname]/cache/label")){
		mkdir(PHP168_PATH."$rsdb[dirname]/cache/label");
		chmod(PHP168_PATH."$rsdb[dirname]/cache/label",0777);
	}
	if($mystyle){
		@include(PHP168_PATH."$rsdb[dirname]/cache/label/{$mystyle}.php");		
		if($i<count($TagDB)){
			if(filesize(PHP168_PATH."$rsdb[dirname]/cache/label/{$mystyle}.php")>1024*1024*1024){
				unlink(PHP168_PATH."$rsdb[dirname]/cache/label/{$mystyle}.php");
			}
			write_file(PHP168_PATH."$rsdb[dirname]/cache/label/{$mystyle}.php","$show\r\n?>",'a');
		}else{
			write_file(PHP168_PATH."$rsdb[dirname]/cache/label/{$mystyle}.php","$show\r\n?>");
		}
	}
...
function check_NewTag(){
	...
	@include(PHP168_PATH."$rsdb[dirname]/cache/label/{$mystyle}.php");
	if(!$inc&&$TagDB[$tag][$ch_pagetype]){
		if($TagDB[$tag][$ch_pagetype]['typesystem']){
			$_inc=str_replace("Info_","",$TagDB[$tag][$ch_pagetype]['type']);
			if($rs=$db->get_one("SELECT * FROM {$pre}module WHERE pre='$_inc'")){
				$inc=$TagDB[$tag][$ch_pagetype]['type'];
			}else{
				$inc='article';
			}
		}elseif($TagDB[$tag][$ch_pagetype]['type']){
			$inc=$TagDB[$tag][$ch_pagetype]['type'];
		}else{
			$inc='article';
		}
	...
	$TagDB[$tag][$ch_pagetype][code]=stripslashes($TagDB[$tag][$ch_pagetype][code]);
	$TagDB[$tag][$ch_pagetype][divcode]=stripslashes($TagDB[$tag][$ch_pagetype][divcode]);
	return $TagDB[$tag][$ch_pagetype];
}
...
function getLabelTpl($type,$type_array=array()){
	global $webdb;	
	if(!is_dir(PHP168_PATH."template/default/label_tpl/$type")){
		$type="common_title";
	}	
	$type_array=array('-1'=>$type)+$type_array;
	foreach( $type_array AS $key=>$value){
		$path=PHP168_PATH."template/default/label_tpl/$value";
		$dir=opendir($path);
		while($file=readdir($dir)){
			if(eregi("\.htm$",$file)){
...
function replace_img(url){
	...
	re   = /$myurl/g;
	link_s = url.replace(re, "http://down2.php168.com");
	return link_s;
}
</SCRIPT>
EOT;
