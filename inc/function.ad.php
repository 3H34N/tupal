...
function make_ad_cache(){
	...
	$show.="<?php\r\n";
	$query = $db->query("SELECT * FROM `{$pre}ad` WHERE isclose=0");
	while($rs = $db->fetch_array($query)){
		...
		if($rs[type]=='word'){
			$url="$webdb[www_url]/do/a_d_s.php?job=jump&id=$rs[id]&u_id=$_r[u_id]&url=".base64_encode($linkurl);
			$code="<SCRIPT LANGUAGE='JavaScript' src='$webdb[www_url]/do/a_d_s.php?job=js&ad_id=$rs[keywords]'></SCRIPT>";
			...
			if($l_src){
				...
				if(eregi("swf$",$l_src)){
					...
					$l_link="$webdb[www_url]/do/a_d_s.php?job=jump&id=$rs[id]&url=".base64_encode($l_link);
					$L_Types="photo";
				}
			}
			if($r_src){
				...
				if(eregi("swf$",$r_src)){
					...
					$r_link="$webdb[www_url]/do/a_d_s.php?job=jump&id=$rs[id]&url=".base64_encode($r_link);
					$R_Types="photo";
				}
			}
...
function close_ad(d){
	...
	write_file(PHP168_PATH."php168/ad_cache.php",$show);
}
