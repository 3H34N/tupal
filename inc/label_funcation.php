...
function add_div($value,$tag,$type='code',$div_w='',$div_h='',$div_bgcolor='',$if_hf=''){
	...
	if($value){
		$div_w>50 || $div_w=50;
		$div_h>21 || $div_h=21;
		$div_bgcolor || $div_bgcolor='blue';
		$type=="flash" && $br="<br>";	//防止被FLASH档住,看不到层
		$value="<div id=\"$tag\" style=\"text-align:left;width:{$div_w}px;height:{$div_h}px;position:absolute;z-index:2;border:#ff0000 solid 1.5px;background-color: $div_bgcolor;filter:Alpha(Opacity=50);\" onmouseover=\"showlabel_(this,'over','$type','$if_hf');\" onmouseout=\"showlabel_(this,'out','$type','$if_hf');\" onclick=\"showlabel_(this,'click','$type','$if_hf');\"><div style='position:absolute;z-index:6;' onclick='return false;'  onmouseover=\"ckjump_(0);\" onmouseout=\"ckjump_(1);\"><img src=\"$webdb[www_url]/images/default/position2.gif\"  style='width:21px;height:21px;' border=\"0\" usemap=\"#Map$tag\"  />
<map name=\"Map$tag\">
<area shape=\"rect\" coords=\"7,0,14,7\" href=\"javascript:\"  onMouseDown=\"change_po_('up',1,'$tag');\"  onMouseUp=\"change_po_('up',0,'$tag');\"  alt=\"向上\" />
<area shape=\"rect\" coords=\"7,14,14,21\" href=\"javascript:\"  onMouseDown=\"change_po_('down',1,'$tag');\" onMouseUp=\"change_po_('down',0,'$tag');\"  alt=\"向下\" />
<area shape=\"rect\" coords=\"0,7,7,14\" href=\"javascript:\"  onMouseDown=\"change_po_('left',1,'$tag');\"  onMouseUp=\"change_po_('left',0,'$tag');\"   alt=\"向左\" />
<area shape=\"rect\" coords=\"14,7,21,14\" href=\"javascript:\" onMouseDown=\"change_po_('right',1,'$tag');\" onMouseUp=\"change_po_('right',0,'$tag');\"  alt=\"向右\" />
</map></div></div>
			$br<table border='0' cellspacing='0' cellpadding='0' class='label_$tag'>
  <tr>
    <td>$value</td>
  </tr>
</table>";
...
function rollPic_JS($rolldb){
	...
	//自定义幻灯片接口
	if($RollStyleType&&is_file(PHP168_PATH."template/default/rollpic/$RollStyleType") ){
		...
		$img=implode("|",$picurl);
		$url=implode("|",$piclink);
		include(PHP168_PATH."template/default/rollpic/$RollStyleType");
		return $show;
	}
	$height=$height+30;	
...
function make_ture_path($format,$rs){
	global $ModuleDB,$webdb,$Html_Type,$showHtml_Type;
	
	if($format[SYS]=='fenlei'){
		...
		if( $ModuleDB[$keyname][id]==$webdb['module_id']&&function_exists('get_info_url') ){
			...
			$list_url.='list.php?city_id=$city_id&fid=$fid';
			$show_url.='bencandy.php?city_id=$city_id&fid=$fid&id=$id';
		}
	...
	//wn指万能文章,normal指5.0版中的图库.视频,下载,FLASH等频道的列表与内容页网址
...
function label_set_rs($format,$rs){
	...
	if($format[SYS]=='pwbbs'){
		...
		if($rs[attachurl]){
			//兼容旧版与非主题有图片的.
			if(is_file(PHP168_PATH."$webdb[passport_path]/$db_attachname/thumb/$rs[attachurl]")){
