...
if($step=='post')
{
	...
	if( $postdb[picurl] && $postdb[picurl]!=$rsdb[picurl] )
	{
		...
		if(file_exists(PHP168_PATH."$webdb[updir]/$postdb[picurl]")&&$postdb[automakesmall]&&$webdb[if_gdimg])
		{
			...
			//多生成一张3:4的图片,方便标签调用
			gdpic(PHP168_PATH."$webdb[updir]/$postdb[picurl]","$Newpicpath.jpg",$picHeight?$picHeight:225,$picWidth?$picWidth:300,$webdb[autoCutSmallPic]?array('fix'=>1):'');

			//多生成一张1:1的图片,方便标签调用
			gdpic(PHP168_PATH."$webdb[updir]/$postdb[picurl]","$Newpicpath.jpg.jpg",$picWidth?$picWidth:300,$picWidth?$picWidth:300,$webdb[autoCutSmallPic]?array('fix'=>1):'');
