...
{	
	if($ckfid){
		...
		echo $shows=get__sonfid($ckfid,$fdivid,0,"{$pre}spsort");
	}
	if(!$shows){
		$showThisId="{$fuid}_".rands(5);
		echo "<select name=\"select\" onchange=\"showfid_S('$showThisId',this,'$fuid','$inputid','$type');\"><option value=\"\" style='color:blue;'>请选择</option>";
		$query = $db->query("SELECT * FROM {$pre}spsort WHERE fup='$fid'");
		while($rs = $db->fetch_array($query)){
			$_r=$db->get_one("SELECT * FROM {$pre}spsort WHERE fup='$rs[fid]'");
			if($_r){
				$rs[fid]=-$rs[fid];
			}
...
function get__sonfid($ckfid,$divid,$num,$table){
	...
	$fdivid="{$fuid}_".rands(5);
	$show.= "<span id=\"$fdivid\" divname='$fuid'><select name=\"select\" onchange=\"showfid_S('$divid',this,'$fuid','$inputid','$type')\"><option value=\"\" style='color:blue;'>请选择</option>";
