...
function system_map($type){
	...
	while($rs = $db->fetch_array($query)){
		...
		if($webdb[NewsMakeHtml]==1){
			...
			if(!$_fiddb[$fid]){
				$showdb[]="<url>\r\n<loc>".replace_url("$webdb[www_url]$webdb[path]/list.php?fid=$fid")."</loc>\r\n</url>";
			}
			$showdb[]="<url>\r\n<loc>".replace_url("$webdb[www_url]$webdb[path]/bencandy.php?fid=$fid&id=$id")."</loc>\r\n</url>";
		}
...
function baidu_map(){
	...
	while($rs = $db->fetch_array($query)){
		if($webdb[NewsMakeHtml]==1){
			...
			$url = replace_url("$webdb[www_url]$webdb[path]/bencandy.php?fid=$rs[fid]&id=$rs[aid]");
		}
