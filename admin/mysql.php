...
elseif($action=='del'&&$Apower[mysql_del]){
	...
	del_file(PHP168_PATH."cache/mysql_bak/$baktime");
	if(!is_dir(PHP168_PATH."cache/mysql_bak/$baktime")){
...
function bak_out($tabledb){
	...
	if(!$rand_dir){
		...
		if( file_exists(PHP168_PATH."cache/mysql_bak/mysql") )
		{
			if( !is_writable(PHP168_PATH."cache/mysql_bak/mysql") ){
				showmsg(PHP168_PATH."cache/mysql_bak/mysql目录不可写,请改属性为0777");
			}
			$rand_dir="mysql";
			copy("mysql_into.php",PHP168_PATH."cache/mysql_bak/$rand_dir/index.php");
			...
			$show=create_table_all($tabledb);	//备份数据表结构
			if( !file_exists(PHP168_PATH."cache/mysql_bak") ){
				if( !@mkdir(PHP168_PATH."cache/mysql_bak",0777) ){
					showmsg(PHP168_PATH."cache/mysql_bak目录不能创建");
				}
			}
			if(	!@mkdir(PHP168_PATH."cache/mysql_bak/$rand_dir",0777)	)
			{
				showmsg(PHP168_PATH."cache/mysql_bak/$rand_dir,目录不可写,请改属性为0777");
			}
	...
	$step=intval($step);
	$filename="$step.sql";
	write_file(PHP168_PATH."cache/mysql_bak/".$rand_dir."/".$filename,$show,'a+');

	//如果不指定每卷大小.将默认为1M
	$baksize=$baksize?$baksize:1024;
	
	//对文件做精确大小分卷处理
	$step=cksize(PHP168_PATH."cache/mysql_bak/".$rand_dir."/".$filename,$step,1024*$baksize);
	...
	if($tabledb[$tableid])
	{
		...
		//记录下来.防止中途备份失败
		write_file(PHP168_PATH."cache/bak_mysql.txt","index.php?lfj=$lfj&action=out&page=$page&rowsnum=$rowsnum&tableid=$tableid&rand_dir=$rand_dir&step=$step&tabledbreto=$Table&baksize=$baksize");
...
EOT;
	...
	{
		$dir=opendir(PHP168_PATH."cache/mysql_bak/$rand_dir");
		while($file=readdir($dir)){
			if($file!='.'&&$file!='..'&&$file!='index.php')
			{
				$totalsize+=$sqlfilesize=@filesize(PHP168_PATH."cache/mysql_bak/$rand_dir/$file");
		...
		$rs[timedir]=$rand_dir;
		if( !@is_writable(PHP168_PATH."cache/mysql_bak/$rand_dir/0.sql") ){
...
function bak_time(){
	...
	$dir=opendir(PHP168_PATH."cache/mysql_bak/");
	while( $file=readdir($dir) ){
		if( is_dir(PHP168_PATH."cache/mysql_bak/$file")&&$file!='.'&&$file!='..' ){
...
function bak_into(){
	...
	$step=intval($step);
	$file=PHP168_PATH."cache/mysql_bak/$baktime/{$step}.sql";
	if( file_exists($file) ){
		$db->insert_file($file);
	}
	$step++;
	if( file_exists(PHP168_PATH."cache/mysql_bak/$baktime/{$step}.sql") ){
		...
		//	write_file(PHP168_PATH."A/$bak_dir",$bak_txt);
		//}
