...
if($job=="list"&&$Apower[form_module])	
{	
	$query = $db->query("SELECT * FROM {$pre}form_module ORDER BY list DESC,id ASC");	
	while($rs = $db->fetch_array($query)){	
		$rss=$db->get_one("SELECT count(*) AS NUM FROM {$pre}form_content WHERE mid='$rs[id]' ");	
		$rs[NUM]=$rss[NUM];	
		$listdb[]=$rs;	
	}	
		
	require("head.php");	
	require("template/form_module/list.htm");	
	require("foot.php");	
}
elseif($job=="set"&&$Apower[form_module])
{
	$rsdb=$db->get_one("SELECT * FROM {$pre}form_module WHERE id='$id'");
	...
	$rsdb[about]=str_replace("'","&#39;",$rsdb[about]);

	$usetitle[$rsdb[usetitle]]=' checked ';
	$repeatpost[$rsdb[repeatpost]]=' checked ';

	require("head.php");	
	require("template/form_module/set.htm");
	require("foot.php");	
}
elseif($action=="set"&&$Apower[form_module])
{
	...
	$db->query("UPDATE {$pre}form_module SET allowpost='$allowpost',allowview='$allowview',endtime='$postdb[endtime]',name='$postdb[name]',about='$postdb[about]',usetitle='$postdb[usetitle]',repeatpost='$postdb[repeatpost]',statename='$postdb[statename]' WHERE id='$id'");
	jump("修改成功","$FROMURL",1);
}
elseif($action=="editlist"&&$Apower[form_module])	
{	
	foreach( $order AS $key=>$value){	
		$db->query("UPDATE {$pre}form_module SET list='$value' WHERE id='$key' ");	
	}
	jump("修改成功","$FROMURL",1);
}	
elseif($action=="addmodule"&&$Apower[form_module])	
{	
	if(!$name){
		showmsg("名称不能为空");
	}
	$rs=$db->get_one("SELECT * FROM `{$pre}form_module` WHERE `name`='$name'");
	if($rs){
		showmsg("当前表单名称已经存在了,请更换一个");
	}
	if(!$IS_BIZPhp168){
		@extract($db->get_one("SELECT COUNT(*) AS NUM FROM {$pre}form_module"));
		if($NUM>21){
	...
	$db->query("INSERT INTO {$pre}form_module (name,config,statename) VALUES ('$name','$config','审核') ");	
	@extract($db->get_one("SELECT id FROM {$pre}form_module ORDER BY id DESC LIMIT 0,1"));	
	unset($SQL);	
	if($dbcharset){	
		$SQL=" DEFAULT CHARSET=$dbcharset ";	
	}	
	$SQL="CREATE TABLE `{$pre}form_content_{$id}` (
	...
	jump("创建成功<br><a href='?lfj=$lfj&job=tpl&id=$id' style='color:red;font-size:25px'>务必注意！！你必须点击生成模板,模块才能生效</a> ","?lfj=$lfj&job=listfield&id=$id");	
}	
	
//修改栏目信息	
elseif($job=="listfield"&&$Apower[form_module])	
{	
	$rsdb=$db->get_one("SELECT * FROM {$pre}form_module WHERE id='$id'");	
	...
	require("head.php");	
	require("template/form_module/listfield.htm");	
	require("foot.php");	
}	
	
elseif($action=="listfield"&&$Apower[form_module])	
{	
	$db->query(" UPDATE {$pre}form_module SET name='$name',style='$Info_style' WHERE id='$id' ");	
		
	jump("修改成功","$FROMURL");	
}	
elseif($action=="editorder"&&$Apower[form_module])	
{	
	$rsdb=$db->get_one("SELECT * FROM {$pre}form_module WHERE id='$id' ");	
	$array=unserialize($rsdb[config]);	
	...
	$db->query("UPDATE {$pre}form_module SET config='$config' WHERE id='$id' ");	
	jump("修改成功<br><a href='?lfj=$lfj&job=tpl&id=$id' style='color:red;font-size:25px'>务必注意！！你必须点击生成模板,模块才能生效</a> ","?lfj=$lfj&job=listfield&id=$id",10);	
}	
elseif($job=="editfield"&&$Apower[form_module])	
{	
	$rsdb=$db->get_one("SELECT * FROM {$pre}form_module WHERE id='$id' ");	
	$array=unserialize($rsdb[config]);	
	...
	require("head.php");	
	require("template/form_module/editfield.htm");	
	require("foot.php");	
}	
elseif($action=="editfield"&&$Apower[form_module])	
{	
	...
	if(table_field("{$pre}form_content",$postdb[field_name])||table_field("{$pre}form_content_$id",$postdb[field_name])){	
	...
	{	
		...
		$db->query("ALTER TABLE `{$pre}form_content_$id` CHANGE `{$field_array[field_name]}` `{$postdb[field_name]}` INT( $postdb[field_leng] ) NOT NULL");	
	}	
	...
	{	
		...
		$db->query("ALTER TABLE `{$pre}form_content_$id` CHANGE `{$field_array[field_name]}` `{$postdb[field_name]}` VARCHAR ( $postdb[field_leng] ) NOT NULL");	
	}	
	elseif($postdb[field_type]=='mediumtext')	
	{	
		$db->query("ALTER TABLE `{$pre}form_content_$id` CHANGE `{$field_array[field_name]}` `{$postdb[field_name]}` MEDIUMTEXT NOT NULL");	
	}	
...
elseif($job=="addfield"&&$Apower[form_module])	
{	
	$rsdb=$db->get_one("SELECT * FROM {$pre}form_module WHERE id='$id' ");	
...
elseif($action=="addfield"&&$Apower[form_module])	
{	
	$postdb[allowview]=implode(",",$postdb[allowview]);	
	if(!ereg("^([a-z])([a-z0-9_]+)$",$postdb[field_name])){	
		showmsg("字段ID不符合规则");	
	...
	{	
		...
		$db->query("ALTER TABLE `{$pre}form_content_$id` ADD `{$postdb[field_name]}` INT( $postdb[field_leng] ) NOT NULL");	
	}	
	elseif($postdb[field_type]=='varchar')	
	{	
		if( $postdb[field_leng]>255 || $postdb[field_leng]<1 ){	
		...
		$db->query("ALTER TABLE `{$pre}form_content_$id` ADD `{$postdb[field_name]}` VARCHAR( $postdb[field_leng] ) NOT NULL");	
	}	
	elseif($postdb[field_type]=='mediumtext')	
	{	
		$db->query("ALTER TABLE `{$pre}form_content_$id` ADD `{$postdb[field_name]}` MEDIUMTEXT NOT NULL");	
	}	
	...
	$config=addslashes(serialize($array));	
	$db->query("UPDATE {$pre}form_module SET config='$config' WHERE id='$id' ");	
	jump("添加成功<br><a href='?lfj=$lfj&job=tpl&id=$id' style='color:red;font-size:25px'>务必注意！！你必须点击生成模板,模块才能生效</a>","index.php?lfj=$lfj&job=listfield&id=$id",10);	
}	
elseif($action=="delfield"&&$Apower[form_module])	
{	
	...
	$config=addslashes(serialize($array));	
	$db->query("UPDATE {$pre}form_module SET config='$config' WHERE id='$id' ");	
	$db->query("ALTER TABLE `{$pre}form_content_$id` DROP `$field_name`");	
	jump("删除成功<br><a href='?lfj=$lfj&job=tpl&id=$id' style='color:red;font-size:25px'>务必注意！！你必须点击生成模板,模块才能生效</a> ",$FROMURL);	
}	
elseif($job=='tpl'&&$Apower[form_module])	
{	
	if($automaketpl){	//批量生成模板
		$autojump="autopost();";
		$page=intval($page);
		$rsdb=$db->get_one("SELECT * FROM {$pre}form_module LIMIT $page,1 ");
		$id=$rsdb[id];
		if(!$id){
			jump("模板生成完毕","index.php?lfj=form_module&job=list",3);
		}
		$page++;
	}else{
		$rsdb=$db->get_one("SELECT * FROM {$pre}form_module WHERE id='$id' ");
	...
	//前台列表页模板
	if(is_file("template/form_module/tpl/list_$id.htm")){
		$list_tpl_file="template/form_module/tpl/list_$id.htm";
	}else{
		$list_tpl_file="template/form_module/tpl/list.htm";
	}
	$list_tpl=str_replace(array('$TempLate1','$TempLate2',"<",">"),array($tpl_L1,$tpl_L2,"&lt;","&gt;"),read_file($list_tpl_file));
	
	//后台列表页模板
	if(is_file("template/form_module/tpl/admin_list_$id.htm")){
		$admin_list_tpl_file="template/form_module/tpl/admin_list_$id.htm";
	}else{
		$admin_list_tpl_file="template/form_module/tpl/admin_list.htm";
	}		
	$admin_list_tpl=str_replace(array('$TempLate1','$TempLate2',"<",">"),array($tpl_L1,$tpl_L2,"&lt;","&gt;"),read_file($admin_list_tpl_file));
	
	//搜索页模板
	if(is_file("template/form_module/tpl/post_$id.htm")){
		$search_tpl_file="template/form_module/tpl/search_$id.htm";
	}else{
		$search_tpl_file="template/form_module/tpl/search.htm";
	}
	$search_tpl=str_replace(array('$TempLate1','$TempLate2',"<",">"),array($tpl_sarch1,$tpl_sarch2,"&lt;","&gt;"),read_file($search_tpl_file));

	//前台发布页模板,没有后台发布页
	if(is_file("template/form_module/tpl/post_$id.htm")){
		$post_tpl_file="template/form_module/tpl/post_$id.htm";
	}else{
		$post_tpl_file="template/form_module/tpl/post.htm";
	}
	$post_tpl=str_replace(array('$TempLate',"<",">"),array($tpl_p,"&lt;","&gt;"),read_file($post_tpl_file));
	
	//前台内容页模板
	if(is_file("template/form_module/tpl/bencandy_$id.htm")){
		$show_tpl_file="template/form_module/tpl/bencandy_$id.htm";
	}else{
		$show_tpl_file="template/form_module/tpl/bencandy.htm";
	}		
	$show_tpl=str_replace(array('$TempLate',"<",">"),array($tpl_s,"&lt;","&gt;"),read_file($show_tpl_file));
	
	//后台内容页模板:
	if(is_file("template/form_module/tpl/admin_bencandy_$id.htm")){
		$admin_show_tpl_file="template/form_module/tpl/admin_bencandy_$id.htm";
	}else{
		$admin_show_tpl_file="template/form_module/tpl/admin_bencandy.htm";
	}		
	$admin_show_tpl=str_replace(array('$TempLate',"<",">"),array($tpl_s,"&lt;","&gt;"),read_file($admin_show_tpl_file));
	
	require("head.php");	
	require("template/form_module/tpl.htm");	
	require("foot.php");	
}	
elseif($action=='tpl'&&$Apower[form_module])	
{	
	...
	if(!is_dir(PHP168_PATH."php168/form_tpl")){	
		makepath(PHP168_PATH."php168/form_tpl");	
	}	
	write_file(PHP168_PATH."php168/form_tpl/post_$id.htm",$tpl_post);

	write_file(PHP168_PATH."php168/form_tpl/search_$id.htm",$tpl_search);

	write_file(PHP168_PATH."php168/form_tpl/bencandy_$id.htm",$tpl_show);
	write_file(PHP168_PATH."php168/form_tpl/list_$id.htm",$tpl_list);

	write_file(PHP168_PATH."php168/form_tpl/admin_bencandy_$id.htm",$admin_tpl_show);
	write_file(PHP168_PATH."php168/form_tpl/admin_list_$id.htm",$admin_tpl_list);

	if(!is_writable(PHP168_PATH."php168/form_tpl/post_$id.htm")){	
		showmsg("php168/form_tpl/post_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if(!is_writable(PHP168_PATH."php168/form_tpl/search_$id.htm")){	
		showmsg("php168/form_tpl/search_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if(!is_writable(PHP168_PATH."php168/form_tpl/bencandy_$id.htm")){	
		showmsg("php168/form_tpl/bencandy_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if(!is_writable(PHP168_PATH."php168/form_tpl/list_$id.htm")){	
		showmsg("php168/form_tpl/list_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}

	if(!is_writable(PHP168_PATH."php168/form_tpl/admin_bencandy_$id.htm")){	
		showmsg("php168/form_tpl/admin_bencandy_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if(!is_writable(PHP168_PATH."php168/form_tpl/admin_list_$id.htm")){	
		showmsg("php168/form_tpl/admin_list_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if($automaketpl){
		jump("请稍候,正在生成下一个模板","index.php?lfj=form_module&job=$action&page=$page&automaketpl=$automaketpl",1);
	}else{
		jump("模板生成完毕","index.php?lfj=form_module&job=listfield&id=$id");
	}		
}	
elseif($action=="delete"&&$Apower[form_module])	
{	
	$rs=$db->get_one("SELECT count(*) AS num FROM {$pre}form_content WHERE mid='$id' ");	
	if($rs[num]){	
		showmsg("本模块频道已有内容了,请先删除内容");	
	}	
	$db->query(" DELETE FROM `{$pre}form_module` WHERE id='$id' ");	
	$id>0 && $db->query(" DROP TABLE IF EXISTS `{$pre}form_content_{$id}`");	

	unlink(PHP168_PATH."template/default/post_$id.htm");	
	unlink(PHP168_PATH."php168/form_tpl/bencandy_$id.htm");	
	unlink(PHP168_PATH."php168/form_tpl/search_$id.htm");

	jump("删除成功","index.php?lfj=form_module&job=list");	
}	
