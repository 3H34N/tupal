...
if($action=='mod'){
	...
	if(strstr($postdb[tplpart_2code],'$picurl')&&!strstr($postdb[tplpart_1code],'$picurl'))
	{
		if(strstr($postdb[tplpart_2code],'$content')){
			$SQL2=" SELECT A.*,A.aid AS id,R.content FROM {$pre}fu_article FA LEFT JOIN {$pre}article$erp A ON FA.aid=A.aid LEFT JOIN {$pre}reply$erp R ON A.aid=R.aid $SQL AND A.ispic=1 AND R.topic=1 ORDER BY $order $asc LIMIT 1 ";
		}else{
			$SQL2=" SELECT A.*,A.aid AS id FROM {$pre}fu_article FA LEFT JOIN {$pre}article$erp A ON FA.aid=A.aid $SQL AND A.ispic=1 ORDER BY $order $asc LIMIT 1 ";
		}		
	}
	...
	if( strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content') ){
		$SQL=" SELECT A.*,A.aid AS id,R.content FROM {$pre}fu_article FA LEFT JOIN {$pre}article$erp A ON FA.aid=A.aid LEFT JOIN {$pre}reply$erp R ON A.aid=R.aid  $SQL $SQLPIC AND R.topic=1 ORDER BY $order $asc LIMIT $rows ";
	}else{
		$SQL=" SELECT A.*,A.aid AS id FROM {$pre}fu_article FA LEFT JOIN {$pre}article$erp A ON FA.aid=A.aid  $SQL $SQLPIC ORDER BY $order $asc LIMIT $rows ";
	}
	...
	$c_rolltypedb[$c_rolltype]=" checked ";
 	...
 	$select_news=$Guidedb->Checkbox("{$pre}fu_sort",'fiddb[]',$fiddb);
