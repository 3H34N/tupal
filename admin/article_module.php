...
elseif($action=="addmodule"&&$Apower[article_module])
{
	...
	if(is_file(PHP168_PATH.'template/default/list_tpl/mod_tpl.htm')){
...
elseif($action=="editsort"&&$Apower[article_module])	
{
	foreach($tpldb AS $key=>$value){
		if($value&&!is_file(PHP168_PATH."$value")){
			showmsg("文件不存在:".PHP168_PATH."$value");
		}
	}
	$rsdb=$db->get_one("SELECT * FROM {$pre}article_module WHERE id='$id' ");	
	$array=unserialize($rsdb[config]);
...
{	
	...
	//后台发表页
	if(is_file(PHP168_PATH."$tpldb[adminpost]")){
		$post_tpl_file=PHP168_PATH."$tpldb[adminpost]";
	...
	//前台会员发表页
	if(is_file(PHP168_PATH."$tpldb[post]")){
		$member_post_tpl_file=PHP168_PATH."$tpldb[post]";
	}elseif(is_file(PHP168_PATH."member/template/tpl/post_$id.htm")){
		$member_post_tpl_file=PHP168_PATH."member/template/tpl/post_$id.htm";
	}else{
		$member_post_tpl_file=PHP168_PATH."member/template/post.htm";
	}
	$member_post_tpl=read_file($member_post_tpl_file);

	//内容页
	if(is_file(PHP168_PATH."$tpldb[show]")){
		$show_tpl_file=PHP168_PATH."$tpldb[show]";
	}elseif(is_file(PHP168_PATH."template/default/tpl/bencandy_$id.htm")){
		$show_tpl_file=PHP168_PATH."template/default/tpl/bencandy_$id.htm";
	}else{
		$show_tpl_file=PHP168_PATH."template/default/bencandy.htm";
	}
	$show_tpl=read_file($show_tpl_file);

	//搜索页
	if(is_file(PHP168_PATH."$tpldb[search]")){
		$search_tpl_file=PHP168_PATH."$tpldb[search]";
	}elseif(is_file(PHP168_PATH."template/default/tpl/search_$id.htm")){
		$search_tpl_file=PHP168_PATH."template/default/tpl/search_$id.htm";
	}else{
		$search_tpl_file=PHP168_PATH."template/default/search.htm";
	}
	$search_tpl=read_file($search_tpl_file);


	//列表页
	if(is_file(PHP168_PATH."$tpldb[list]")){
		$list_tpl_file=PHP168_PATH."$tpldb[list]";
...
{	
	...
	$tpl_list && write_file(PHP168_PATH."template/default/list_tpl/mod_$id.htm",$tpl_list);
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr> <td >{$rs[title]}:$mustfill<br>{$rs[form_title]}  <a href='javascript:showinput_{$rs[field_name]}()'>添加远程视频</a></td> <td >
		<!--\r\nEOT;\r\n
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr> <td >{$rs[title]}:$mustfill<br>{$rs[form_title]} <a href='javascript:showinput_{$rs[field_name]}()'>添加远程文件</a></td> <td>
<!--\r\nEOT;\r\n
\$upfiletype=str_replace(' ',',',trim(\$groupdb[upfileType]?\$groupdb[upfileType]:\$webdb[upfileType]));
\$upfiletype=str_replace('.','',\$upfiletype);
\$max_upload=ini_get('upload_max_filesize')?ini_get('upload_max_filesize'):'0';
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr> <td >{$rs[title]}:$mustfill<br>{$rs[form_title]}<a href='javascript:showinput_{$rs[field_name]}()'>添加远程图片</a></td> <td >
