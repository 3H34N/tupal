...
elseif($action=="addpic"&&$Apower[artic_addpic])
{
	...
	foreach($photodb AS $key=>$photo){
		...
		if($batch==0||$ck==0)
		{
			...
			$db->query("INSERT INTO {$pre}reply$erp (  `aid` , `fid` ,`uid` ,  `content` ,`topic`) VALUES ( '$rs[aid]', '$postdb[fid]','$userdb[uid]', '$postdb[content]','1')");
		...
		else
		{
			...
			$db->query("INSERT INTO {$pre}reply$erp (  `aid` , `fid` ,`uid` ,  `content`) VALUES ( '$rs[aid]', '$postdb[fid]','$userdb[uid]', '$postdb[content]')");
			$II++;
		}
		$ck++;
	}
	...
	jump("<CENTER>[<A HREF='index.php?lfj=artic&job=addpic&fid=$postdb[fid]&bak_id=$postdb[bak_id]'>继续发表新组图</A>] [<A HREF='index.php?lfj=artic&job=myarticle&fid=$postdb[fid]'>返回文章列表</A>] [<A HREF='$webdb[www_url]$webdb[path]/bencandy.php?fid=$rs[fid]&aid=$rs[aid]' target=_blank>查看文章</A>] [<A HREF='index.php?lfj=post&job=manage&id=$rs[aid]'>点击修改</A>]</CENTER>","index.php?lfj=artic&job=postnew&fid=$postdb[fid]",600);
}
...
elseif($job=="listartic"&&$Apower[artic_listartic])
{
	...
	$sort_fid=$Guidedb->Select("{$pre}sort","fid",$fid,"$webdb[admin_url]/index.php?lfj=$lfj&job=listartic");
...
elseif($job=="work"&&$Apower[artic_listartic])
{
	...
	if($jobs=="move"){
		...
		$fusort_select=$Guidedb->Select("{$pre}fu_sort","fid");
	}
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/artic/article_menu.htm");
