...
@include(PHP168_PATH."php168/module.php");
...
function write_config_cache($webdbs)
{
	...
	if(!is_writable(PHP168_PATH."php168/config.php")){
		showmsg(PHP168_PATH."php168/config.php文件目录权限不可写,请仔细检查");
	}
}
...
function module_menu(){
	...
	if($Smenu)
	{
		...
		@include(PHP168_PATH."{$ModuleDB[$Smenu][dirname]}/{$ModuleDB[$Smenu][admindir]}/menu.php");
...
function get_htmltype(){
	...
	write_file(PHP168_PATH."php168/htmltype.php","<?php\r\n".$show.'?>');
}
...
function write_limitword_cache(){
	...
	write_file(PHP168_PATH."php168/limitword.php","$show");
}
...
function write_keyword_cache(){
	...
	write_file(PHP168_PATH."php168/keyword.php","$show");
}
...
function write_friendlink(){
	...
	while($rs2 = $db->fetch_array($query2)){
		unset($logodb,$worddb);
		$query = $db->query("SELECT * FROM {$pre}friendlink WHERE fid='$rs2[fid]' AND yz=1 AND (endtime=0 OR endtime>$timestamp) ORDER BY list DESC");
		while($rs = $db->fetch_array($query)){
	...
	write_file(PHP168_PATH."php168/friendlink.php",$write);
}
...
function article_module_cache(){
	...
	while($rs = $db->fetch_array($query)){
		$rs[config] = addslashes( serialize(  unserialize($rs[config])  ) );	//特意的
	...
	write_file(PHP168_PATH."php168/article_module.php",$show."?>");
}
?>