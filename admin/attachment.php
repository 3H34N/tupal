...
if($job=="listimg"&&$Apower[attachment_list]){
	$imgdb=array();
	get_imgfile(PHP168_PATH."$webdb[updir]");
	krsort($imgdb);
	if($page<1){
		$page=1;
	}
	...
	foreach($imgdb AS $key=>$value){
		...
		$listdb[]=array('time'=>date("m-d H:i",$key),'name'=>basename($value),'path'=>str_replace(PHP168_PATH,"",$value));
	}
	$showpage=getpage("","","?lfj=$lfj&job=$job","$rows",$total=count($imgdb));
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/attachment/listimg.htm");
