	...
	if($type=='myatc'||$type=='all')
	{
		...
		while($rs = $db->fetch_array($query)){
			...
			$show.="<tr align='center' class='trA' onmouseover=\"this.className='trB'\" onmouseout=\"this.className='trA'\"> 
                ...
                <td width='84%' align='left'><A HREF='..$webdb[path]/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target=_blank>{$rs[title]}</A></td>
                <td width='11%'>&nbsp;$add</td>
              </tr>";
		}
		$show="<table width='100%' border='0' cellspacing='1' cellpadding='3'>
		...
		$showpage=str_replace("href=&page=","target=spiframe href=special.php?job=show_iframe&id=$id&fid=$fid&type=myatc&ismy=$ismy&search_type=$search_type&keywords=".urlencode($keywords)."&page=",$showpage);
		echo "<SCRIPT LANGUAGE='JavaScript'>
		<!--
...
elseif($job=="show_BBSiframe"){
	...
	if($type=='myatc'||$type=='all')
	{
		...
		$showpage=str_replace("\n","",$showpage);
		$showpage=str_replace('"',"",$showpage);
		$showpage=str_replace("href=&page=","target='spiframe' href=special.php?job=show_BBSiframe&id=$id&fid=$fid&type=myatc&ismy=$ismy&keywords=".urlencode($keywords)."&page=",$showpage);
		echo "<SCRIPT LANGUAGE='JavaScript'>
		<!--
		parent.document.getElementById('show_myatc_page').innerHTML=\"$showpage\";
		//-->
