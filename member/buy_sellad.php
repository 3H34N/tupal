...
if($job=='buy')
{
	...
	$_s2=$db->get_one("SELECT COUNT(*) AS Num FROM `{$pre}sellad_user` WHERE id='$id' AND endtime>$timestamp");
	...
	if($step){
		...
		$db->query("INSERT INTO `{$pre}sellad_user` ( `uid` , `username` , `begintime` , `endtime` , `money` , `id`, `adlink`, `adword`) VALUES ('$lfjdb[uid]','$lfjdb[username]','$timestamp','$postdb[endtime]','$postdb[price]','$id','$postdb[adlink]','$postdb[adword]')");
		add_user($lfjuid,-$postdb[price]);	//扣除积分
...
elseif($job=='list')
{
	...
	$query = $db->query("SELECT AD.* FROM `{$pre}sellad` AD ORDER BY AD.id DESC");
...
elseif($job=='sell_list')
{
	...
	$min=($page-1)*$rows;
	$showpage=getpage("`{$pre}sellad_user`","WHERE id='$id'","?job=$job&id=$id","$rows");
	$query = $db->query("SELECT * FROM `{$pre}sellad_user` WHERE id='$id' ORDER BY ad_id DESC LIMIT $min,$rows");
	while($rs = $db->fetch_array($query)){
...
elseif($job=='mylist')
{
	$query = $db->query("SELECT AD.name,B.* FROM `{$pre}sellad` AD LEFT JOIN `{$pre}sellad_user` B ON AD.id=B.id WHERE B.uid='$lfjuid' ORDER BY B.ad_id DESC");
	while($rs = $db->fetch_array($query)){
		$rs[begintime]=date("Y-m-d H:i",$rs[begintime]);
		$rs[endtime]=date("Y-m-d H:i",$rs[endtime]);
		$listdb[]=$rs;
...
elseif($action=='del')
{
	$db->query("DELETE FROM `{$pre}sellad_user` WHERE uid='$lfjuid' AND ad_id='$ad_id'");
	refreshto("$FROMURL","删除成功",1);
}
elseif($job=='edit')
{
	$rsdb = $db->get_one("SELECT AD.name,AD.wordnum,B.* FROM `{$pre}sellad` AD LEFT JOIN `{$pre}sellad_user` B ON AD.id=B.id WHERE B.uid='$lfjuid' AND ad_id='$ad_id'");
	...
	if($step==2){
		...
		$postdb[adlink]=filtrate($postdb[adlink]);
		$postdb[adword]=filtrate($postdb[adword]);
		$db->query("UPDATE `{$pre}sellad_user` SET adword='$postdb[adword]',adlink='$postdb[adlink]' WHERE  uid='$lfjuid' AND ad_id='$ad_id'");
...
require(dirname(__FILE__)."/"."template/buy_sellad/list.htm");
